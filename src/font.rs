...
/// The classic public-domain 5x7 ASCII font (Adafruit-GFX's `glcdfont.c`),
/// covering `0x20..=0x7E` plus a box glyph at `0x7F` used as a fallback for
/// unknown/non-ASCII characters.
///
/// Each glyph is 5 column bytes; within a column byte, bit 0 is the top row
/// and bit 6 is the bottom row. Bit 7 is never drawn: Adafruit's table is
/// really 5x8 and puts the descenders of `g`, `p`, `q` and `y` there, so
/// those four glyphs are redrawn here to fit in seven rows. Everything else
/// is byte-for-byte the reference table.
...
mod tests {
    ...
    /// Every glyph that fits in seven rows must be the Adafruit `glcdfont.c`
    /// one. These were the ones that had drifted: `>` was a column off from
    /// `<`, `[`/`]` were two columns wide, and several letters were freehand.
    #[test]
    fn glyphs_match_adafruit_glcdfont() {
        let expected: [(char, [u8; 5]); 18] = [
        ...
        for (c, bytes) in expected {
            assert_eq!(*glyph_for(c), bytes, "glyph {c:?}");
        }
    }

    /// Adafruit draws g/p/q/y with a descender in an eighth row this font
    /// does not have, so they keep 7-row shapes and never set bit 7.
    #[test]
    fn descender_glyphs_stay_within_seven_rows() {
        for c in ['g', 'p', 'q', 'y'] {
            assert!(glyph_for(c).iter().all(|column| column & 0x80 == 0), "glyph {c:?}");
        }
    }
